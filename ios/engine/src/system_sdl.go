...
func (s *System) newWindow(w, h int) (*Window, error) {
	...
	if runtime.GOOS == "android" {
		...
		if renderName == "OpenGL ES 3.2" || renderName == "OpenGL ES 3.0" {
...
func (w *Window) SwapBuffers() {
	// On iOS, SDL's UIKit swap presents whatever renderbuffer is currently
	// bound to GL_RENDERBUFFER. The engine's renderer binds its own depth
	// renderbuffer, so we must rebind SDL's on-screen color renderbuffer
	// before presenting, otherwise the presented buffer is the depth buffer
	// (black) and the rendered scene never reaches the screen.
	// (On non-iOS this is 0, which is a harmless no-op/break of the binding.)
	gl.BindRenderbuffer(gl.RENDERBUFFER, iosSdlColorRenderbuffer)
	w.Window.GLSwap()
}
...
func (w *Window) pollEvents() {
	...
	// On iOS, drain native on-screen control events so the game consumes them
	// this frame.
	iosTouchUpdateInput()
}
