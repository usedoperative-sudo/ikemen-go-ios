// This is render_gles32.go adapted for iOS (OpenGL ES 3.0 via
// github.com/leonkasovan/gl/v3.0/gles2). Model/headbone/shadowmap
// rendering is disabled (enableModel=false) because those paths
// require ES 3.2 features.
...
import (
	...
	gl "github.com/leonkasovan/gl/v3.0/gles2"
...
// SDL's on-screen color renderbuffer (viewRenderbuffer) on iOS. SDL presents
// whatever renderbuffer is bound to GL_RENDERBUFFER at swap time, so this
// must be rebound before every present. Captured right after the GL context
// is made current, before we bind any of our own renderbuffers.
var iosSdlColorRenderbuffer uint32
...
func (r *Renderer_GLES32) newShaderProgram(vert, frag, geo, name string, crashWhenFail bool) (s *ShaderProgram_GLES32, err error) {
	...
	// Geometry shaders are not available in OpenGL ES on iOS, so they
	// are ignored here. No render path in this build uses them anyway
	// (model rendering is disabled).
	if len(geo) > 0 {
		Logcat("GLES: Warning: geometry shader requested for " + name + ", ignoring on iOS")
	}
...
func (r *Renderer_GLES32) compileShader(shaderType uint32, src string) (uint32, error) {
	...
	if !strings.HasPrefix(strings.TrimSpace(src), "#version") {
		// iOS supports OpenGL ES 3.0 (GLSL ES 3.00). Use 300 es for
		// maximum compatibility since iOS has no ES 3.2/3.20 support.
...
// Renderer_IOS is the OpenGL ES 3.0 renderer used on iOS.
// It is implemented by Renderer_GLES32 (see render_ios.go).
type Renderer_IOS = Renderer_GLES32
...
type Renderer_GLES32 struct {
	...
	// SDL's on-screen drawable framebuffer on iOS. On iOS, framebuffer 0 is
	// NOT the drawable (it has no color attachment); SDL creates a dedicated
	// FBO and binds it when the GL context is made current. We capture its
	// name here and bind it when we want to present to the screen.
	fboScreen uint32
...
func (r *Renderer_GLES32) GetName() string {
	// iOS only supports OpenGL ES up to 3.0 (Apple discourages OpenGL in
	// favor of Metal), so report 3.0 to get a matching GL context.
...
func (r *Renderer_GLES32) Init() {
	chk(gl.InitWithProcAddrFunc(iosGetProcAddress))
	Logcat(fmt.Sprintf("Using OpenGL %v (%v)", gl.GoStr(gl.GetString(gl.VERSION)), gl.GoStr(gl.GetString(gl.RENDERER))))

	// Capture SDL's on-screen drawable framebuffer. SDL_GL_MakeCurrent binds
	// it right before this; nothing else has bound an FBO yet, so
	// GL_FRAMEBUFFER_BINDING currently holds the drawable's name. On iOS this
	// is NOT 0 (framebuffer 0 has no color attachment there).
	var fboBinding int32
	gl.GetIntegerv(gl.FRAMEBUFFER_BINDING, &fboBinding)
	r.fboScreen = uint32(fboBinding)
	Logcat(fmt.Sprintf("PRESENT: captured SDL drawable framebuffer = %d (note: iOS FB0=nothing)", r.fboScreen))

	// Capture SDL's on-screen color renderbuffer. Right now (right after the
	// context was made current, before we bind our own) the currently-bound
	// renderbuffer is SDL_uikitopenglview.viewRenderbuffer, which is what SDL
	// presents at swap time. We must rebind it right before each present.
	var rboBinding int32
	gl.GetIntegerv(gl.RENDERBUFFER_BINDING, &rboBinding)
	iosSdlColorRenderbuffer = uint32(rboBinding)
	Logcat(fmt.Sprintf("PRESENT: captured SDL color renderbuffer = %d", iosSdlColorRenderbuffer))
	...
	// r.fbo_pp_texture
	// Post-processing FBO color attachments. On iOS (OpenGL ES 3.0),
	// RGBA8_SNORM is not in the required color-renderable format list,
	// which makes these FBOs incomplete and renders nothing to screen.
	// Use plain RGBA8 which is guaranteed color-renderable in ES 3.0.
	for i := 0; i < 2; i++ {
	...
	if sys.msaa > 0 {
		...
		gl.FramebufferRenderbuffer(gl.FRAMEBUFFER, gl.DEPTH_ATTACHMENT, gl.RENDERBUFFER, r.rbo_depth)
		if status := gl.CheckFramebufferStatus(gl.FRAMEBUFFER); status != gl.FRAMEBUFFER_COMPLETE {
			LogMessage("Main FBO creation failed: 0x%x", status)
		} else {
			LogMessage("Main FBO complete")
		}
	}
	...
	for i := 0; i < 2; i++ {
		...
		if status := gl.CheckFramebufferStatus(gl.FRAMEBUFFER); status != gl.FRAMEBUFFER_COMPLETE {
			LogMessage("Postprocess FBO %d creation failed: 0x%x", i, status)
		} else {
			LogMessage("Postprocess FBO %d complete", i)
		}
	}

	// An FBO for model stuff is not needed on iOS (model rendering is disabled).
...
func (r *Renderer_GLES32) EndFrame() {
	...
	for i := 0; i < len(r.postShaderSelect); i++ {
		...
		if i >= len(r.postShaderSelect)-1 {
			...
			// to the on-screen (SDL drawable) framebuffer
			x, y, width, height := sys.window.GetScaledViewportSize()
			// On iOS the drawable is in pixels, not points, and we want the
			// render to fill the whole screen (stretch), like the desktop.
			if runtime.GOOS == "ios" {
				x, y = 0, 0
				width, height = sys.window.GLGetDrawableSize()
			}
			gl.Viewport(x, y, width, height)
			// Bind the SDL on-screen drawable FBO (NOT 0 — on iOS FB0 has no
			// color attachment and rendering to it produces black).
			gl.BindFramebuffer(gl.FRAMEBUFFER, r.fboScreen)
...
func (r *Renderer_GLES32) prepareModelPipeline(bufferIndex uint32, env *Environment) {
	...
	if r.enableShadow {
		// NOTE: shadow cube maps require ES 3.2 (TEXTURE_CUBE_MAP_ARRAY).
		// Shadows are disabled on iOS, so this path is never reached.
	}
	if env != nil {
...
func (r *Renderer_GLES32) SetShadowFrameTexture(i uint32) {
	// Avoid an ES 3.2-only call (glFramebufferTexture).
	_ = i
}
